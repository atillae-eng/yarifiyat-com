export default function Personalization({ campaigns }: { campaigns: any[] }) {
  ...
  return (
    <section className="mb-12 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-3xl p-6 border-2 border-blue-300 shadow-sm">
      ...
      <p className="text-gray-600 mb-3 text-sm">İlgi alanlarına göre seçtiğimiz fırsat:</p>
      <div className="bg-white rounded-xl shadow p-4 flex items-center justify-between hover:shadow-md transition">
        <div>
          <div className="font-bold text-gray-800">{random?.brand_name}</div>
          <div className="text-sm text-gray-500">%{random?.discount} indirim</div>
          <div className="text-xs text-blue-500 font-medium">{random?.category}</div>
        </div>
        <a href={random?.affiliate_url} target="_blank" rel="noopener" 
           className="bg-blue-500 text-white px-4 py-2 rounded-xl text-sm hover:bg-blue-600 transition shadow">
          Fırsatı Gör
        </a>
      </div>
    </section>
